...
for thread_num in range(1, thread + 1):
    with open(abspath + '/Code/Thread/D1_FY3D_Thread{0}.py'.format(thread_num), 'w') as f:
        f.write('''import os
import datetime
import numpy as np
import xarray as xr
...
num = {2}
interval = int(len(date_list) / {3}) + 1
                    ...
                    if '{{0}}-{{1}}-{{2}}'.format(year, month, day) in date_list:
                        ...
                        if os.path.exists(save_path) == False:
                            ...
                            try:
                                ...
                                bt = file.groups['Calibration']['EARTH_OBSERVE_BT_10_to_89GHz']
                                lat = file.groups['Geolocation']['Latitude']
                                lon = file.groups['Geolocation']['Longitude']
                                saa = file.groups['Geolocation']['Sensor_Azimuth']
                                mask = file.groups['Calibration']['LandSeaMask']
                                ...
                                lat = np.delete(lat, np.where(bt_all[-1] == -32767), axis = 0)
                                lon = np.delete(lon, np.where(bt_all[-1] == -32767), axis = 0)
                                saa = np.delete(saa, np.where(bt_all[-1] == -32767), axis = 0)
                                for channel_num in range(10):
                                    bt_all[channel_num] = np.delete(bt_all[channel_num], np.where(bt_all[-1] == -32767), axis = 0)
                                for channel_num in range(10):
                                    bt_all[channel_num] = np.delete(bt_all[channel_num], np.where(lat == 65535.0), axis = 0)
                                lon = np.delete(lon, np.where(lat == 65535.0), axis = 0)
                                saa = np.delete(saa, np.where(lat == 65535.0), axis = 0)
                                lat = np.delete(lat, np.where(lat == 65535.0), axis = 0)
                                for channel_num in range(10):
                                    bt_all[channel_num] = np.delete(bt_all[channel_num], np.where(lon == 65535.0), axis = 0)
                                lat = np.delete(lat, np.where(lon == 65535.0), axis = 0)
                                saa = np.delete(saa, np.where(lon == 65535.0), axis = 0)
                                lon = np.delete(lon, np.where(lon == 65535.0), axis = 0)
                                for channel_num in range(10):
                                    bt_all[channel_num] = np.delete(bt_all[channel_num], np.where(saa == 65535), axis = 0)
                                lat = np.delete(lat, np.where(saa == 65535), axis = 0)
                                lon = np.delete(lon, np.where(saa == 65535), axis = 0)
                                saa = np.delete(saa, np.where(saa == 65535), axis = 0)
                                ...
                                if len(points_grid) != 0:
                                    ...
                                    print('FY3D_{{0}}_{{1}}.{{2}}.{{3}}.{{4}}.{{5}}'.format(orbit, year, month, day, hour, minute))
                            except:
                                error = 'N'
                                print('ERROR-FY3D_{{0}}_{{1}}.{{2}}.{{3}}.{{4}}.{{5}}'.format(orbit, year, month, day, hour, minute))
print(error)'''.format(begin_date, end_date, thread_num, thread))

for thread_num in range(1, thread + 1):
    os.system('start cmd.exe cmd /k D:/SOFTWARE/Anaconda3/envs/python310/python.exe {0}/Code/Thread/D1_FY3D_Thread{1}.py'.format(abspath, thread_num))