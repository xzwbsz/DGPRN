...
for thread_num in range(1, thread + 1):
    with open(abspath + '/Code/Thread/D1_FY3C_Thread{0}.py'.format(thread_num), 'w') as f:
        f.write('''import os
import datetime
import numpy as np
import xarray as xr
                    ...
                    if '{{0}}-{{1}}-{{2}}'.format(year, month, day) in date_list:
                        ...
                        if os.path.exists(save_path) == False:
                            ...
                            try:
                                ...
                                saa = file.groups['Geolocation']['SensorAzimuth']
                                ...
                                for channel_num in range(10):
                                    bt_all[channel_num] = np.delete(bt_all[channel_num], np.where(lat == 999.999), axis = 0)
...
for thread_num in range(1, thread + 1):
    os.system('start cmd.exe cmd /k D:/SOFTWARE/Anaconda3/envs/python310/python.exe {0}/Code/Thread/D1_FY3C_Thread{1}.py'.format(abspath, thread_num))