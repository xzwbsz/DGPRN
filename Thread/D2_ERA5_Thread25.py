...
num = 1
interval = int(len(date_list) / 10) + 1
date_list = date_list[int(interval * (num - 1)):int(interval * num)]
loc_era5 = 'I:/ERA5/'
                ...
                if '{0}-{1}-{2}'.format(year, month, day) in date_list:
                    ...
                    if os.path.exists(save_path) == False:
                        ...
                        for sswsst_str in ['ussw', 'vssw', 'sst']:
                            ...
                            sswsst = np.array(sswsst)
                            sswsst = griddata(points, sswsst.reshape(-1, 1), (lon_grid.reshape(-1, 1), lat_grid.reshape(-1, 1)), method = 'linear')[:, :, 0][:, 0]
