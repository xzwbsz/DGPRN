...
for thread_num in range(1, thread + 1):
    with open(abspath + '/Code/Thread/D2_ERA5_Thread{0}.py'.format(thread_num), 'w') as f:
        f.write('''import os
import datetime
import numpy as np
import xarray as xr
...
loc_era5 = 'I:/ERA5/'
loc_fy3 = abspath + '/DataSet/X/FY3'
for orbit in ['asc', 'dsc']:
    file_list = loc_fy3 + '/{{0}}'.format(orbit.capitalize())
            ...
            if (file.split('_')[1] == orbit) and (file.split('_')[-1][:4] == str(year)):
                ...
                if '{{0}}-{{1}}-{{2}}'.format(year, month, day) in date_list:
                    ...
                    if os.path.exists(save_path) == False:
                        ...
                        bt = fy3['bt']
                        lat_grid = np.array(fy3['lat'])
                        lon_grid = np.array(fy3['lon'])
                        ...
                        print('ERA5_{{0}}_{{1}}.{{2}}.{{3}}.{{4}}.{{5}}'.format(orbit, year, month, day, hour, minute))'''.format(begin_date, end_date, thread_num, thread))

for thread_num in range(1, thread + 1):
    os.system('start cmd.exe cmd /k D:/SOFTWARE/Anaconda3/envs/python310/python.exe {0}/Code/Thread/D2_ERA5_Thread{1}.py'.format(abspath, thread_num))