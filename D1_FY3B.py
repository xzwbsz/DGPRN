...
for thread_num in range(1, thread + 1):
    with open(abspath + '/Code/Thread/D1_FY3B_Thread{0}.py'.format(thread_num), 'w') as f:
        f.write('''import os
import datetime
import numpy as np
import xarray as xr
...
for thread_num in range(1, thread + 1):
    os.system('start cmd.exe cmd /k D:/SOFTWARE/Anaconda3/envs/python310/python.exe {0}/Code/Thread/D1_FY3B_Thread{1}.py'.format(abspath, thread_num))